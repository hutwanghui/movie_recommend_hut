...
<script>
  ...
  export default {
    ...
    methods: {
      fetchMovie(id) {
        axios.get(`https://api.themoviedb.org/3/movie/${id}?api_key=${storage.apiKey}&language=zh-cn`)
          .then(function (resp) {
            let movie = resp.data
            this.movie = movie
            this.poster()
      ...
      checkIfInFavorites(id) {
        ...
        API.movie.checkIfFavoriteMovies(params)
          .then(function (resp) {
      ...
      toggleFavorite() {
        ...
        API.movie.addOrRemoveFavorite(params)
          .then(function (resp) {
