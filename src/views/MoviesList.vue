  ...
  <div class="wrapper" v-if="listLoaded">
    ...
    <section v-if="!movies.length" class="not-found">
      <div class="not-found__content">
        ...
        <h2 class="not-found__title" v-if="mode == 'userRecommend'">Your frinends haven't added any movies</h2>
        <h2 class="not-found__title" v-if="mode == 'modelRecommend'">System haven't added any Recommend movies</h2>
      </div>
    </section>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      request() {
        if (this.mode == 'search') {
          return `https://api.themoviedb.org/3/search/movie?api_key=${storage.apiKey}&language=zh-cn&query=${this.query}&page=${this.currentPage}`
        } else if (this.mode == 'collection') {
          let caregory = this.$route.params.category || this.category
          return `https://api.themoviedb.org/3/movie/${caregory}?api_key=${storage.apiKey}&language=zh-cn&page=${this.currentPage}`
        } else if (this.mode == 'favorite') {
          return requestUrl('api/movie/favorite/get/?username=' + storage.userId)
        }else if(this.mode == 'userRecommend'){
          return requestUrl('api/movie/userRecommend/get/?username='+storage.userId)
        }else if(this.mode == 'modelRecommend'){
          return requestUrl('api/movie/modelRecommend/get/?username='+storage.userId)
        }
      },
      countResults() {
        if (this.results > 1) {
    ...
    created() {
      ...
      if (this.mode == 'search') {
        ...
        this.$root.eventHub.$emit('setSearchQuery')
      } else if (this.mode == 'collection') {
        let caregory = this.$route.params.category || this.category
        ...
        this.listTitle = storage.categories['userRecommend']
      }else if(this.mode == 'modelRecommend'){
        this.listTitle = storage.categories['modelRecommend']
      }
      this.fetchCategory()
